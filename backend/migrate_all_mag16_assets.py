#!/usr/bin/env python3
"""
Migrate all 234 MAG16 assets from Excel to database
"""
...
def extract_mag16_from_excel():
    ...
    try:
        ...
        # Find Bloomberg ID and Par Amount columns (we know they're at row 2)
        ...
        if assets:
            ...
            for i, asset in enumerate(assets[:5]):
                print(f"  {i+1}. {asset['bloomberg_id']}: ${asset['par_amount']:,.2f}")
...
def migrate_to_database(assets):
    ...
    try:
        with db_config.get_db_session('postgresql') as db:
            ...
            # Check if assets exist in main assets table, if not we need to create them
            print("Checking main assets table...")
            existing_assets = db.execute(text("""
                SELECT bloomberg_id FROM assets 
            """)).fetchall()
            existing_blk_ids = {row[0] for row in existing_assets}
            print(f"Found {len(existing_blk_ids)} existing assets in main table")
            
            # Insert new assets into main assets table if needed
            new_assets_needed = []
            for asset in assets:
                if asset['bloomberg_id'] not in existing_blk_ids:
                    new_assets_needed.append(asset)
            
            if new_assets_needed:
                print(f"Creating {len(new_assets_needed)} new assets in main table...")
                for i, asset in enumerate(new_assets_needed):
                    db.execute(text("""
                        INSERT INTO assets (
                            asset_id, bloomberg_id, par_amount, 
                    ...
                    """), {
                        'asset_id': f"MAG16_ASSET_{i+1:03d}",
                        'bloomberg_id': asset['bloomberg_id'],
                        'par_amount': asset['par_amount'],
                        'created_at': datetime.now(),
                        'updated_at': datetime.now()
                    })
                
                if len(new_assets_needed) % 50 == 0:
                    db.commit()
                    print(f"Progress: {len(new_assets_needed)} assets created in main table")
            
            # Insert all assets into deal_assets table
            ...
            for asset in assets:
                try:
                    ...
                    print(f"Error inserting asset {asset['bloomberg_id']}: {e}")
                    continue
...
def main():
    ...
    print("MAG16 Complete Asset Migration")
    print("=" * 50)
