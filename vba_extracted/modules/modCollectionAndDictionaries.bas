Attribute VB_Name = "modCollectionAndDictionaries"
Option Explicit
Option Private Module
Option Compare Text
''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
' modSortACollectionOrDictionary
...
If IsArray(Arr) = False Then
    ...
    'PerfMonProcEnd "VBAProject.modQSortInPlace.QSortCompare"
    Exit Function
End If
...
If SortByKey = True Then
    ...
    For Ndx = 0 To Dict.Count - 1
        VTypes(Ndx) = VarType(Dict.Keys(Ndx))
    ...
    For Ndx = 0 To Dict.Count - 1
            ...
            Case vbBoolean
                TempKeyvalue = CBool(Arr(Ndx))
            Case vbByte
                TempKeyvalue = CByte(Arr(Ndx))
            Case vbCurrency
                TempKeyvalue = CCur(Arr(Ndx))
            Case vbDate
                TempKeyvalue = CDate(Arr(Ndx))
            Case vbDecimal
                TempKeyvalue = CDec(Arr(Ndx))
            Case vbDouble
                TempKeyvalue = CDbl(Arr(Ndx))
            Case vbInteger
                TempKeyvalue = CInt(Arr(Ndx))
            Case vbLong
                TempKeyvalue = CLng(Arr(Ndx))
            Case vbSingle
                TempKeyvalue = CLng(Arr(Ndx))
            Case vbString
                TempKeyvalue = CStr(Arr(Ndx))
            Case Else
                TempKeyvalue = ItemValue
        End Select
    
    
        TempDict.Add Key:=TempKeyvalue, Item:=Dict.Item(TempKeyvalue)
    Next Ndx
    '''''''''''''''''''''''''''''''''
