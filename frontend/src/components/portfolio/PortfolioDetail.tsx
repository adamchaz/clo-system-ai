...
import type { ConcentrationTestResult } from '../../store/api/newApiTypes';
import ConcentrationTestsPanel from './ConcentrationTestsPanel';
import { getTestName, getTestCategory } from '../../utils/concentrationTestMappings';
...
const transformConcentrationDataForDisplay = (apiData: any) => {
  ...
  // Check if we have the new database-driven ConcentrationTestResult format
  if (apiData && apiData.concentration_tests && Array.isArray(apiData.concentration_tests)) {
    
    const tests = apiData.concentration_tests.map((test: any) => {
      const status = test.pass_fail === 'PASS' ? 'PASS' as const : 
                    test.pass_fail === 'FAIL' ? 'FAIL' as const : 'WARNING' as const;
      
      // Use database-driven test information with proper mappings
      // Map to snake_case properties expected by ConcentrationTestsPanel component
      return {
        test_number: test.test_number,
        test_name: getTestName(test.test_number), // Use mapped test name from frontend mappings
        category: test.category || getTestCategory(test.test_number) || 'general', // Use mapped category
        result: test.result, // Use database-provided result directly
        threshold: test.threshold, // Use database-provided threshold directly
        threshold_operator: test.threshold_operator || '<=', // Use database-provided operator
        pass_fail: status,
        risk_level: status === 'FAIL' ? 'CRITICAL' as const : status === 'WARNING' ? 'HIGH' as const : 'LOW' as const,
        comments: test.comments || (status === 'PASS' ? 'Within limits' : status === 'FAIL' ? 'Exceeds limit' : 'Near limit'),
        recommendation: status === 'FAIL' ? 'Review concentration immediately and consider rebalancing' : 
                       status === 'WARNING' ? 'Monitor closely for potential breach' : undefined,
        numerator: test.numerator || 0,
        denominator: test.denominator || 1,
        last_updated: new Date().toISOString(),
        // Enhanced threshold properties from database-driven backend
        threshold_source: test.threshold_source || 'database',
        is_custom_override: test.is_custom_override || false,
        effective_date: test.effective_date || apiData.analysis_date,
        mag_version: test.mag_version // Include MAG version from database
      };
    });
    ...
    return {
      tests: tests,
      summary: {
        totalTests: apiData.total_tests || tests.length,
        passing: apiData.passed_tests || tests.filter(t => t.pass_fail === 'PASS').length,
        failing: apiData.failed_tests || tests.filter(t => t.pass_fail === 'FAIL').length,
        warnings: apiData.summary?.warning_tests || tests.filter(t => t.pass_fail === 'WARNING').length,
        complianceScore: apiData.summary?.compliance_score || `${Math.round((tests.filter(t => t.pass_fail === 'PASS').length / tests.length) * 100)}%`
      }
    };
  }
  
  // Handle old ConcentrationAnalysisResult format as fallback
  if (apiData && apiData.concentration_metrics) {
    // HHI tests removed - not part of Excel file requirements
    const pseudoTests = [];
    
    return {
      tests: pseudoTests,
      summary: {
        totalTests: pseudoTests.length,
        passing: pseudoTests.filter(t => t.status === 'PASS').length,
        failing: pseudoTests.filter(t => t.status === 'FAIL').length,
        warnings: pseudoTests.filter(t => t.status === 'WARNING').length,
        complianceScore: `${Math.round((pseudoTests.filter(t => t.status === 'PASS').length / pseudoTests.length) * 100)}%`
      }
    };
  }
  
  // No real database-driven data available - return empty state
  ...
  return {
    ...
    error: 'Concentration test data not available. Please ensure the database-driven concentration test service is running.'
  };
};

// NOTE: All hardcoded VBA TestNum mappings removed
// Now using database-driven concentration test approach exclusively
...
const PortfolioDetail: React.FC<PortfolioDetailProps> = ({
  ...
  // Real concentration tests mutation (new endpoint with actual test results)
  const [runConcentrationTests, { isLoading: concentrationTestsLoading }] = useRunConcentrationTestsMutation();

  const portfolio = portfolioData;
  const summary = portfolioSummary;

  // Run real concentration tests using the new backend integration service
  const runConcentrationAnalysis = useCallback(async () => {
    ...
    try {
      const result = await runConcentrationTests(request).unwrap() as ConcentrationTestResult;
      setConcentrationData(result);
    } catch (error) {
      console.error('❌ Real concentration tests failed:', error);
      // Fallback to original analysis if needed
      try {
        ...
        const result = await analyzeConcentration(fallbackRequest).unwrap();
        setConcentrationData(result);
      } catch (fallbackError) {
        console.error('❌ Fallback concentration analysis also failed:', fallbackError);
      }
    }
  }, [portfolio, portfolioId, runConcentrationTests, analyzeConcentration, concentrationTestsLoading]);

  const handleTabChange = useCallback((event: React.SyntheticEvent, newValue: number) => {
    setCurrentTab(newValue);
    
    // Run concentration analysis when Compliance tab (index 5) is selected
    if (newValue === 5 && !concentrationData && !concentrationLoading && !concentrationTestsLoading) {
        ...
        {concentrationLoading ? (
          ...
          <Box sx={{ textAlign: 'center', py: 10 }}>
            ...
            <Typography variant="body2" color="text.secondary" sx={{ mb: 3 }}>
              Click "Run Real Tests" to load REAL concentration tests with live portfolio data from the backend ConcentrationTestIntegrationService for {portfolioId}
            </Typography>
            <Button
              variant="contained"
              onClick={runConcentrationAnalysis}
              startIcon={<Security />}
              disabled={concentrationLoading || concentrationTestsLoading}
            >
              {(concentrationLoading || concentrationTestsLoading) ? 'Running Real Tests...' : 'Run Real Concentration Tests'}
            </Button>
          </Box>
        )}
      </TabPanel>
