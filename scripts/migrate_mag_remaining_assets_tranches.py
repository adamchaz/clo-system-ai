#!/usr/bin/env python3
"""
Migrate Assets and Tranches for Remaining MAG Deals: MAG8, MAG9, MAG12, MAG14, MAG15
Focus on assets and tranches migration without changing database schema

This script:
1. Migrates asset data from Excel input worksheets to deal_assets table
2. Creates tranche structures in clo_tranches table
3. Verifies migration results
"""
...
# Deal configurations based on Excel analysis
DEAL_CONFIGS = {
    'MAG8': {
...
def load_deal_assets(deal_id):
    ...
    try:
        # Read with header on row 3 (index 2) - confirmed from debug
...
def migrate_deal_assets(conn, deal_id, excel_df):
    ...
    # Check which new assets exist in assets table
    total_migrated = 0
    if new_ids:
        ...
        logger.info(f"  {len(valid_new_ids)} new assets found in assets table")
        
        # Insert new records
        insert_records = []
        for blkrock_id in valid_new_ids:
    ...
    if update_ids:
        ...
        if update_records:
            ...
            total_migrated += len(update_records)
    
    conn.commit()
    logger.info(f"Successfully migrated {total_migrated} {deal_id} assets")
...
def migrate_deal_tranches(conn, deal_id):
    ...
    for i, tranche in enumerate(tranches):
        record = {
            'tranche_id': f"{deal_id}-{tranche['name'].replace(' ', '').replace('-', '')}",
            'deal_id': deal_id,
...
def verify_deal_migration(conn, deal_id):
    ...
    logger.info(f"{deal_id} RESULTS:")
    logger.info(f"  Assets: {asset_result['asset_count']}")
    logger.info(f"  Par Amount: ${actual_par:,.2f}")
    logger.info(f"  Coverage: {coverage:.1f}%")
    logger.info(f"  Tranches: {tranche_result['tranche_count']}")
    logger.info(f"  Tranche Total: ${float(tranche_result['total_tranches']):,.2f}")
    
    return {
        'asset_count': asset_result['asset_count'],
        'total_par': actual_par,
        'coverage': coverage,
        'tranche_count': tranche_result['tranche_count'],
        'status': 'COMPLETE' if coverage >= 90 and tranche_result['tranche_count'] >= 6 else 'PARTIAL'
    }

def main():
    """Main migration function"""
    logger.info("Starting Assets and Tranches Migration for Remaining MAG Deals...")
    ...
    try:
        ...
        for deal_id in ['MAG8', 'MAG9', 'MAG12', 'MAG14', 'MAG15']:
            logger.info(f"\n{'='*15} MIGRATING {deal_id} {'='*15}")
            
            try:
                # 1. Load and migrate assets
                excel_df = load_deal_assets(deal_id)
                assets_migrated = migrate_deal_assets(conn, deal_id, excel_df)
                
                # 2. Migrate tranches
                tranches_migrated = migrate_deal_tranches(conn, deal_id)
                ...
                migration_results[deal_id] = {
                    ...
                    'tranches_migrated': tranches_migrated,
                ...
                migration_results[deal_id] = {'error': str(e), 'success': False}
        ...
        for deal_id, results in migration_results.items():
            ...
            total_assets += results['assets_migrated']
            total_tranches += results['tranches_migrated']
            
            logger.info(f"{deal_id}: {status} - {verification['asset_count']} assets, "
                       f"{verification['tranche_count']} tranches, {verification['coverage']:.1f}% coverage")
        
        logger.info(f"\nOVERALL RESULTS:")
        logger.info(f"  Total assets migrated: {total_assets}")
        logger.info(f"  Total tranches created: {total_tranches}")
        logger.info(f"  Complete deals: {complete_deals}/5")
        logger.info(f"  Failed deals: {failed_deals}/5")
        
        if complete_deals == 5:
            logger.info(f"\n✅ ALL REMAINING MAG DEALS MIGRATION COMPLETED!")
            logger.info("All 10 MAG deals now have complete assets and tranches")
        else:
            logger.warning(f"\n⚠️  {failed_deals} deals had errors")
